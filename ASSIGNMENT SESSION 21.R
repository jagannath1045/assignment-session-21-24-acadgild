...
write(rules, "C:/Users/Jagannath/Documents/assignment data acadgild/assignmnet 21-24/rules2.csv", sep=",")
...
# What are customers likely to buy if they purchased "Product A"
rules<-apriori(data=dt, parameter=list(supp=0.001,conf = 0.8), 
               appearance = list(default="rhs",lhs="0"),
               control = list(verbose=F))
## Warning in asMethod(object): removing duplicated items in transactions
rules<-sort(rules, decreasing=TRUE,by="confidence")
inspect(rules[1:5])
...
data1<-na.exclude(data)
na.omit(data1)
##  [ reached getOption("max.print") -- omitted 15717 rows ]
data1.active <- data1[2:100, 2:6]
na.exclude(data1.active)
View(data1.active)
head(data1.active[, 2:5])
...
## additional trials 
library(readr)
epi_r<- read.csv("C:/Users/Jagannath/Documents/assignment data acadgild/assignmnet 21-24/epi_r.csv")
...
biplot(p,pc.biplot=TRUE, col=c("black","red"), cex=c(0.9,0.8),xlim=c(-2.5,2.5), xlab="PC1, 39.7% explained variance", ylab="PC2, 24.5% explained variance")
